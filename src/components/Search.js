...
const SearchInput = ({ handleSearch }) => {
  ...
  return (
    <>
      <form
        className="w-full lg:w-96 relative flex items-center ml-0 lg:ml-5 font-nunito"
      ...
      {searchText.length > 0 && (
        <ul
          className="absolute top-11 left-0 lg:left-5  w-full lg:w-96 h-80 lg:h-96 rounded
    overflow-x-hidden py-2 bg-gray-200 bg-opacity-60
    backdrop-blur-md scrollbar-thin scrollbar-thumb-gray-100 scrollbar-track-gray-200 z-50
    "
        >
          {searchData ? (
            searchData.map((coin) => (
